...
async fn handle_connection(stream: TcpStream) {
    let addr = stream.peer_addr().unwrap_or_else(|_| "unknown:0".parse().unwrap());
    info!("New connection: {addr}");

    let ws = match accept_async(stream).await {
        Ok(ws) => ws,
        Err(e) => { error!("Handshake WS failed: {e}"); return; }
    };
    info!("WebSocket OK with {addr}");

    let (mut write, mut read) = ws.split();
    // message de bienvenue
    let _ = write.send(Message::Text("Welcome".into())).await;
    ...
    while let Some(msg) = read.next().await {
        match msg {
            ...
            Ok(Message::Close(_)) | Ok(Message::Ping(_)) | Ok(Message::Pong(_)) => break,
            Err(e) => { error!("Error WS {addr}: {e}"); break; }
            _ => {}
        }
    }
    info!("Connexion closed: {addr}");
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let listener = TcpListener::bind("127.0.0.1:8080").await?;
    info!("Echo WS on ws://127.0.0.1:8080");
...
async fn handle_client(stream: TcpStream, mut rx: broadcast::Receiver<PriceUpdate>) {
    ...
    info!("Client connected : {addr} | assets = {curr}");

    let ws = match accept_async(stream).await {
        Ok(ws) => ws,
        Err(e) => {
            error!("Handshake WS failed: {e}");
    ...
    let hello = serde_json::json!({
        "type": "connected",
        "message": "Real time feed",
        "active": ACTIVE.load(Ordering::SeqCst),
    });
    let _ = write.send(Message::Text(hello.to_string().into())).await;
    loop {
        tokio::select! {
            Ok(upd) = rx.recv() => {
                if let Ok(txt) = serde_json::to_string(&upd) {
            ...
            msg = read.next() => {
                match msg {
                    Some(Ok(Message::Text(txt))) => {
                        if txt.trim().eq_ignore_ascii_case("/stats") {
                           ...
                           let _ = write.send(Message::Text(payload.to_string().into())).await;
                            ...
                            info!("Client {addr} says: {txt}");
                        }
                    }
                    Some(Ok(Message::Close(_))) | None => break,
                    Some(Err(e)) => { warn!("WS err {addr}: {e}"); break; }
                    _ => {}
                }
            }
        }
    ...
    let curr = ACTIVE.fetch_sub(1, Ordering::SeqCst) - 1;
    info!("Client disconnected: {addr} | assets = {curr}");
}
...
async fn price_simulator(tx: broadcast::Sender<PriceUpdate>) {
    ...
    loop {
        ...
        info!("Broadcast: {} @ {:.2} ({}) | actifs={}",
              update.symbol, update.price, update.source, ACTIVE.load(Ordering::SeqCst));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn({
        let tx = tx.clone();
        async move { price_simulator(tx).await }
    });

    let listener = TcpListener::bind("127.0.0.1:8080").await?;
    info!("Broadcast WS on ws://127.0.0.1:8080");
...
use std::env;
use tokio::{net::TcpListener, sync::broadcast, time::{interval, Duration}};
use tokio_tungstenite::{accept_async, tungstenite::Message};
...
async fn poll_database(pool: &PgPool, tx: &broadcast::Sender<PriceUpdate>) -> Result<(), sqlx::Error> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_url = env::var("DATABASE_URL")
        .unwrap_or_else(|_| "postgresql://postgres:password@localhost:5432/stockdb".to_string());
